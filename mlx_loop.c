...
** Supports both native and WASM (Emscripten) builds
*/
...
if (WindowShouldClose() || g_xvar->end_loop)
g_xvar->end_loop = true;
#endif
}

int mlx_loop(void *mlx_ptr)
{
g_xvar = (t_xvar *)mlx_ptr;
if (!g_xvar || !g_xvar->initialized)
return (0);
g_first_frame = true;
g_xvar->end_loop = false;

#ifdef __EMSCRIPTEN__
/* For WASM: Use Emscripten's main loop
** 0 = use browser's requestAnimationFrame (recommended)
** 1 = simulate infinite loop (required for mlx_loop behavior)
*/
emscripten_set_main_loop(mlx_loop_iteration, 0, 1);
#else
/* For native: Traditional while loop */
while (!WindowShouldClose() && !g_xvar->end_loop)
{
mlx_loop_iteration();
}
#endif
